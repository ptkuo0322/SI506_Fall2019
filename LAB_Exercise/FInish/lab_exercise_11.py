...
def get_films(url, params=None):
    ...
    Returns:
        films(str) :a list of returned films
...
print(get_films.__doc__) # via target.__doc__, we can print the docstring of the target.
...
def filter_properties(film):
    ...
    Extract properties from film

    Parameters:
        film (dict): a dictionary containing properties of the film

    Returns:
        record(dict) : a dictionary includes corresponding key-value pairs of film in file_properties
...
def main():
    """
    write a list of records for films into a JSON file.
