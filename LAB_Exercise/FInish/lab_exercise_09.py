...
with open('john.json','w') as file:
    file.write(json_dict2)
# json_dict2 IS A json object,
# when we expect to write into json file, we would use the file extention .json 
# and write the json object into it
...
with open(csv_name) as file:
    ...
    for i in csv_content:
        if row > 0:
            animals.append((i[0] , i[1]))
            # animails.append(tuple(i))
        row += 1
...
with open('animals2.csv', 'w') as file:
    csv_content = csv.writer(file, delimiter = ',')
    csv_content.writerow(["id", "name"])
    for i in animals:
        #csv_content.writerow(list(i))
        csv_content.writerow([i[0], i[1]])
