...
for x in range(len(great_wall_list)):
    great_wall_item_length.append(len(great_wall_list[x]))
...
# BEGIN PROBLEM 4 SOLUTION
    ...
    great_wall_string = ''.join(f"{great_wall_list[-3]} is in {great_wall_list[-4]}")
...
for sites in china_unesco_sites:
    if sites.split(',')[-2] == "Cultural":
        unesco_sites.append(f"{sites.split(',')[0]} - {sites.split(',')[1]} - {sites.split(',')[3]}")
