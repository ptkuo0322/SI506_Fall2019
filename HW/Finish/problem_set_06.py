...
4. After you have completed the above steps, check the output of the provided print statements to verify the outcomes.
5. [Note] No code will be written for this problem.
[Explanations & Hints]
We highly highly recommend that you use the terminal commands that we cover this week to create all the folders.
...
# Setup Start
"""
If you have set up the folder correctly, you should see at least the following strings displayed on the terminal when you uncomment displayFolderAndFile('.') and execute the script in your terminal.
...
2. Open the file using source_path, and read each line within the file as a separate string. Store these strings in sequence into a list named file_lines_list.
...
first_two_lines_list = file_lines_list[0:2]
...
[Instructions]
1. Define two functions named transform_a_list_even() and transform_a_list_odd(). Each function will take a list of strings as the first and only argument, and return a transformed list of strings as the returned value
...
def transform_a_list_even(list_of_strings):
    ...
    for i in range(len(list_of_strings)):
        if (i+1)%2==0:
            list_even.append(list_of_strings[i].replace("- ",""))
    return list_even
...
# Setup Start
transformed_file_lines_list_odd = []
transformed_file_lines_list_even = []
# Setup End
...
2. Combine the strings from transformed_file_lines_list_odd and transformed_file_lines_list_even into a list of strings and write them as a sequence of lines into the file target.txt, using target_path.
2.1 Each string will be in the format of "<nth odd string>|<nth even string>". For instance, the 1st string in target.txt will be the result of combining the 1st odd string and the 1st even string into "<1st odd string>|<1st even string>".
2.2 Before combining two strings, remember to remove the newline character, "\n", from the string in transformed_file_lines_list_odd, so that the new string will only have one newline character at the end.
3. Check the content of the target.txt to see if your code behaves correctly. The target.txt should contain a number of lines that is half of the source.txt.
[Explanations & Hints]
You are likely to use some of the following concepts or functions to solve this problem:
* file operation and function
...
with open(target_path,'w') as target:
    for i in range(len(transformed_file_lines_list_odd)):
        first_element=transformed_file_lines_list_odd[i].replace('\n','')
        second_element=transformed_file_lines_list_even[i]
        target.write(f"{first_element}|{second_element}")
...
def output_words_frequency_from_file(source_file_path, target_file_path, topic_words_list):
    ...
    with open(source_file_path,'r') as source:
        for line in source:
            file_lines_list.append(line)
    transform_list=transform_a_list_odd(file_lines_list)
    for str in transform_list:
        str2=str.replace("\n","")
        str2=str2.split()
        clean_list.append(str2)
    for element in clean_list:
        for word in topic_words_list:
            if word in element:
                count += element.count(word)
