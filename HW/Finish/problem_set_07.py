...
dictionary_one = {"name":"William Shakespeare","born":1564,"died":1616,"age":2}
print("12345")
# Update <age> with <died> - <born>
# e.g. <died> = 2019, <born> = 2000, then <age> = 19
dictionary_one["age"]= 52
...
wrd_of_lyrics = lyrics.lower().split()
for wrd in wrd_of_lyrics:
    words_frequency[wrd]+= 1
...
#char_frequency=char_frequency.fromkeys(key,0)
#dic.fromkey()用法：dic.fromkey(sequence, value)
#快速生成一個dic的方法，seqence會被默認成dict中的key, 而value則是被默認成value值
#如果沒有預設value值，則系統自動default as None
char_frequency = {"a":0,"b":0,"c":0,"d":0,"e":0,"f":0,"g":0,"h":0,"i":0,"j":0,"k":0,"l":0,"m":0,"n":0,"o":0,"p":0,"q":0,"r":0,"s":0,"t":0,"u":0,"v":0,"w":0,"x":0,"y":0,"z":0}
long_st_lyrics = long_lyrics.lower()
long_st_lyrics_1 = long_st_lyrics.replace('\n',' ')
long_st_lyrics_2 = long_st_lyrics_1.replace(',',' ')
long_st_lyrics_3 = long_st_lyrics_2.replace(' ','')
long_string_list = list(long_st_lyrics_3)
for char in long_string_list:
...
def find_most_common_char(long_string):
    char_frequency = {"a":0,"b":0,"c":0,"d":0,"e":0,"f":0,"g":0,"h":0,"i":0,"j":0,"k":0,"l":0,"m":0,"n":0,"o":0,"p":0,"q":0,"r":0,"s":0,"t":0,"u":0,"v":0,"w":0,"x":0,"y":0,"z":0}
    long_string_1 = long_string.lower()
    long_string_2 = long_string_1.replace('\n',' ')
    long_string_3 = long_string_2.replace(',',' ')
    long_string_4 = long_string_3.replace(' ','')
    long_string_list = list(long_string_4)
    for char in long_string_list:
        char_frequency[char] += 1

    max_char_count=0
    max_char=''
    for char in char_frequency: #默認iterate through key
        if char_frequency[char]>max_char_count: # char_frequency[char]對應的value值
            max_char_count=char_frequency[char]
            max_char=char
    return max_char
