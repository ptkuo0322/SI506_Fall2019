...
import json
powers={}
with open("sh_powers.json","r") as json_file:
    powers=json.load(json_file)
...
for line in info_data:
    key=get_info(line,info_header)["name"]
    value=get_info(line,info_header)
    for item in powers:
        if item==key:
            value["powers"]=powers[item]
    heroes[key]=value
...
with open ("heroes.json", "w") as json_file:
    json.dump(heroes, json_file, indent=2)
...
def get_team(teamname, data=heroes):
    teammates=[]
    for item in heroes:
        dic=heroes[item]
        team=dic["team"]
...
for hero in heroes:
    ...
    power=dic["powers"]
    base=dic["base"].lower()
    if "super_strength" in power and "new york city" in base:
        nyc_strong_heroes.append(hero)
# END PROBLEM 6
